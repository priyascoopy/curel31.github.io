  ...
  slideshow.addEventListener("scroll", () => {
    if (!ticking) {
      requestAnimationFrame(() => {
        const imageToReorder = [...slideshow.children].reduce((prev, current) => ~direction ? ((prev.style.order < current.style.order) ? prev : current) : ((prev.style.order > current.style.order) ? prev : current));
